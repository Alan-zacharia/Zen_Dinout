...
const ForgotPasswordComponent: React.FC = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (credentials: credentials) => {
       await axios.put(`http://localhost:3000/api/reset-password/${id}`,{credentials}).then((response)=>{
    ...
    <div className="m-auto h-[500px] shadow-lg shadow-gray-400  w-[450px] flex justify-center relative" >
      ...
      >
        <div className="text-start pb-5 ">
          <h1 className="text-2xl font-bold text-red-500 absolute top-10 left-7">x Reset Password</h1>
        </div>
  ...
  <p className="text-red-500 font-semibold text-sm flex max-w-[370px]">
    {formik.errors.password}
        ...
        {formik.touched.confirmPassword && formik.errors.confirmPassword && (
          <p className="text-red-500 font-semibold text-sm flex max-w-[370px]">{formik.errors.confirmPassword}</p>
        )}
          <input
            type="password"
            className="p-3 outline-none border border-b-neutral-700 text-sm  w-full "
